...
def run(newsletter_date: date | None = None) -> None:
    ...
    emails = gmail_ops.list_messages(max_results=MAX_EMAILS, query=NEWSLETTER_QUERY)
    if not emails:
        logger.info("No unread Medium newsletter emails found. Done.")
        return

    logger.info("Found %d newsletter email(s) to process.", len(emails))
    ...
    for email_meta in emails:
        ...
        if not html:
            logger.warning(
                "Message %s had no HTML body — marking processed anyway.", message_id
            )
